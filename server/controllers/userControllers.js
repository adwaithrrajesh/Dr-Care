...
module.exports = {
  ...
  getBookingDetails: async (req, res) => {
    ...
    await AppointmentModel.find({ doctorId: doctor, slot: { $gt: 0 } })
      .then((response) => {
  ...
  initializePayment: async (req, res) => {
    ...
    if(bookingExisted){
      res.status(404).json({message:"You already booked an appointment on this date"})
    }else{
    ...
    if (razorpay_signature === expectedSignature) {
      userHelper
        ...
        .then((response) => {
          res
            .status(200)
            .json({ message: "Successfully Booked an Appointment" });
        })
        .catch((error) => {
  ...
  updateProfileDetails: async (req, res) => {
    ...
    const profileUpdateDetails = Object.fromEntries(
  ...
  getBookedAppointments: async (req, res) => {
    try {
      ...
      const appointments = await bookedAppointmentModel.find({ userId: userId, cancelled: false }).populate("doctorId").sort({ date: -1 }).populate("userId");
  ...
  cancelAppointment: async (req, res) => {
    try {
      ...
      await bookedAppointmentModel
        .updateOne({ _id: bookedAppointmentId }, { $set: { cancelled: true } })
        .then(async (response) => {
          ...
          // Increasing Slot
          const slotIncrease = await AppointmentModel.findByIdAndUpdate(
            appointmentId,
            { $inc: { slot: 1 } },
            { new: true }
          );
          if (slotIncrease) {
            const refundMoney = await userModel.findByIdAndUpdate(
              userId,
            ...
            res
              .status(200)
              .json({
                message: `Appointment Cancelled Successfully and ₹${amountPaid} credited to your wallet`,
              });
          } else {
  ...
  getWalletBalance: async (req, res) => {
    try {
      ...
      const user = await userModel.findOne({
        _id: jwt?.verify(token, process.env.JWT_SECRET_KEY)?.userId,
      });
      ...
      res
        .status(404)
        .json({ message: "An unknown error Occured Please try again later" });
    }
  },
  ...
  getCancelledAppointments: async (req, res) => {
    const userId = req.userId;
    const appointments = await bookedAppointmentModel
      .find({ userId: userId, cancelled: true })
      .populate("doctorId");
    res.status(200).json({ appointments: appointments });
  },
  ...
  getChattableDoctors: async (req, res) => {
    try {
    const userId = req.userId;
    const doctorIds = await bookedAppointmentModel.distinct("doctorId", { userId })
    const doctors = await doctorModel.find({ _id: doctorIds });
    res.status(200).json({chattableDoctors:doctors}) 
    } catch (error) {
      res.status(500).json({message:"Internal Server Error"})
    }
  },
